# --- START OF FILE main_button_dev.py ---
...
favicon_link = Link(rel="icon", type="image/png", href="/images/coherent_atom_symbol.png")
...
def simple_message_html(content, role):
    ...
    if not is_user:
        ...
        cleaned_content = re.sub(r"```\s*\n\s*```", "", content) # Example
        cleaned_content = re.sub(r"```[a-z]*\s*\n\s*```", "", cleaned_content) # Example
        try:
            from mistletoe import Document, HTMLRenderer
            doc = Document(cleaned_content)
            renderer = HTMLRenderer()
            content_html = renderer.render(doc)
            content_html = re.sub(r"<pre>\s*<code>\s*</code>\s*</pre>", "", content_html)
        except Exception as e:
            logging.warning(f"Markdown failed: {e}.", exc_info=False)
            content_html = f"<p>{content}</p>"
    else:
    ...
    return Div( # id="chat-container"
        ...
        Div( # Form and controls area
            Form(
                Div( # Input Row
                    ...
                    Button(
                        Span( # Outer Span (button-inner-content)
                            Span(Span(cls="spinner-square"), cls="inline-spinner"), # Spinner Span
                        ...
                        type="submit", id="run-stop-button", cls="run-button-base",
                        style="position: absolute; right: 6px; top: 50%; transform: translateY(-50%); overflow: hidden;",
                ...
                hx_post="/send-message", hx_target="#chat-messages", hx_swap="beforeend",
                hx_indicator="#chat-form", id="chat-form",
            ), # End Form
    ...
    return Titled(
        ...
        Div(
            ...
            Div( # Container for Logo
                Img(
                    src="/images/Coherent_logo_blue.png", alt="Coherent Logo",
                    style="height: 60px; width: auto; display: block; margin-left: 40px; margin-bottom: 10px; margin-top: 20px;"
                ),
...
# --- END OF FILE main_button_dev.py ---

...
def create_parser() -> LlamaParse:
    ...
    return LlamaParse(
        ...
        adaptive_section_parsing=True  # Enable adaptive section parsing
    )
        ...
        for attempt in range(max_retries):
            try:
                ...
                if doc and len(doc) > 0:
                    logging.info(f"Successfully parsed {fname.name} in {elapsed:.2f} seconds")
                    logging.info(f"Content length: {len(doc)}")
                    return doc
                else:
                    logging.info(f"No content returned on attempt {attempt + 1}")
            except Exception as e:
                logging.error(f"Error on attempt {attempt + 1} for {fname.name}: {str(e)}")
...
if __name__ == "__main__":
    ...
    # Process documents in parallel
    results = asyncio.run(process_documents_parallel(
        file_list, parser_template, max_workers=args.max_workers, timeout_seconds=args.timeout
    ))
